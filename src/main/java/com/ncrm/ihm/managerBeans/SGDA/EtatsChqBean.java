...
@ManagedBean(name = "Etats_ChqSGDABean")
@RequestScoped
 ...
 public void editetatsChq(){
	    ...
	    try {
			...
			if(selectedpcapts.length>0 && selectedTrts.length>0){
				 ...
				 list= new ArrayList(Arrays.asList(selectedpcapts));
...
//				{
//
//					logger.debug("===    Generation des etats [ Traitement= "+t.getNom_Traitement()+"]");
//					
//					for(BeanPcapt p:listpcaptSelected) 
//					{
//						
...
//								
//									
//									logger.debug("@@@    [ REPORT AGE = "+rep.getId_report()+" ]");
//									parameters.put("age_rattachement",p.getAgeRattachement());
//									parameters.put("flag_scan_gr","0");
//									
//									metier.ReportPdf(rep.getModeleJRXML(),parameters, null, outDir,namePdfFile);
...
//					}
//				}
//				else  if (t.getNom_Traitement().equalsIgnoreCase("ETA_CHQ_SGMA_GR"))
//				{
//
//					logger.debug("===    Generation des etats [ Traitement= "+t.getNom_Traitement()+"]");
//					
...
//				}
//				
//				else if  (t.getNom_Traitement().equalsIgnoreCase("ETA_CHQ_SGMA_CRT"))
//				{
//					
//					for(BeanReport rep:metier.getReportByTrt(LblBq, t.getIdT()))
//					{
...
//			}
 ...
 public void editRcWc(){
	 ...
	 List<RecordRemise> recordRems=metier.getRecordRemRec(LblBq, cdeTr, dteInstance);
		...
		metier.createFileRec(recordRems, nameFile, header, cdeTr);
	 
//	System.out.println("recordRems.size()() "+recordRems.size());
	 for(RecordRemise r:recordRems)
	 {
		 System.out.println("####> Remise : "+r.getPkObjIdtRem()+" ## >");
		 
		 for (RecordValue c:r.getRecordsValue()){
			 System.out.println("**> Chèque : "+c.getPkObjIdt()+" ** >");
		 }
	 }
